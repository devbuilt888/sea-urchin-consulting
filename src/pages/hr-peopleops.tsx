...
const features = [
  {
    ...
    desc: 'Workflows for docs, training, and compliance.'
  },
  {
    icon: '📊',
    title: 'Sentiment Dashboards',
    desc: 'Pulse surveys and engagement analytics.'
  },
  {
    icon: '🤖',
    title: 'AI Candidate Screening',
    desc: 'Faster, smarter hiring and scheduling.'
  },
  {
    icon: '🔗',
    title: 'Unified HR Systems',
    desc: 'Integrate recruiting, payroll, and performance.'
  }
];
...
const steps = [
  {
    icon: '🔍',
    title: '1. Discovery',
    desc: 'We review your HR stack and pain points.'
  },
  {
    icon: '📄',
    title: '2. Workflow Automation',
    desc: 'We automate onboarding and docs.'
  },
  {
    icon: '📊',
    title: '3. Engagement Analytics',
    desc: 'We set up dashboards and pulse surveys.'
  },
  {
    icon: '🤖',
    title: '4. AI Screening',
    desc: 'We enable smart candidate screening and scheduling.'
  }
];

const testimonials = [
  {
    quote: '“Onboarding is a breeze and we finally have real engagement data. Our HR team is so much happier!”',
    author: 'HR Director, Growth Startup'
  }
];
  ...
  return (
    <main className="min-h-screen text-gray-900 font-sans overflow-x-hidden relative" style={{ background: 'radial-gradient(ellipse at 60% 40%, #a78bfa 0%, #60a5fa 60%, #fff 100%)' }}>
      ...
      <section className="relative flex flex-col justify-center items-center text-center px-6 pt-32 pb-24 overflow-hidden">
        <h1 className="text-5xl font-bold mb-4 max-w-2xl relative z-10 text-purple-600 drop-shadow">Modern HR & People-Ops Solutions</h1>
        <h6 className="text-lg text-blue-600 mb-2 relative z-10">Automation, Analytics, and AI for High-Growth Teams</h6>
        <p className="text-xl max-w-xl mb-8 text-blue-700 relative z-10">
          VP People & HR Directors at high-growth startups. $5K–$50K/project. Automate onboarding, measure engagement, and unify your HR stack.
        </p>
        <a
          href="#leadform"
          className="px-8 py-4 bg-purple-500 text-white rounded-full shadow-lg hover:bg-purple-600 hover:scale-105 transition-transform duration-200 font-semibold text-lg relative z-10"
        >
          Book a Free Demo
        </a>
      </section>
      ...
      <section className="flex flex-col items-center px-4 mb-12">
        <div className="bg-purple-100/90 border-l-4 border-blue-400 shadow-lg rounded-xl px-8 py-6 max-w-2xl w-full flex flex-col items-center">
          <span className="uppercase text-xs tracking-widest text-blue-500 font-bold mb-2">Pain Points</span>
          <div className="text-lg font-semibold text-blue-700 mb-1">Onboarding paperwork overload</div>
          <div className="text-lg font-semibold text-blue-700 mb-1">Hard-to-measure employee engagement</div>
          <div className="text-lg font-semibold text-blue-700 mb-1">Fragmented HR systems (recruiting, payroll, performance)</div>
        </div>
      </section>
      ...
      <section ref={featuresRef} className={`py-20 px-6 text-center transition-all duration-700 ${featuresVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
        <h2 className="text-3xl font-semibold mb-12 text-purple-600">Everything You Need for People-Ops</h2>
        <div className="grid md:grid-cols-4 gap-8 max-w-6xl mx-auto">
          {features.map((f, idx) => (
            <div
              key={f.title}
              className={`bg-white/90 rounded-2xl shadow-lg p-8 flex flex-col items-center hover:shadow-2xl hover:scale-105 transition-transform duration-200 border-t-4 border-purple-200 transition-all duration-700 ${featuresVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
      ...
      <section ref={stepsRef} className={`py-20 px-6 bg-gradient-to-br from-purple-100 via-blue-50 to-white text-center shadow-inner rounded-2xl max-w-5xl mx-auto my-16 transition-all duration-700 ${stepsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
        <h2 className="text-3xl font-semibold mb-12 text-purple-600">How It Works</h2>
        <div className="flex flex-col md:flex-row items-center justify-center gap-5 md:gap-0 relative">
          {steps.map((step, i) => (
            <div key={step.title} className={`flex flex-col items-center flex-1 min-w-[120px] relative z-10 group transition-all duration-700 ${stepsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: stepsVisible ? `${i * 0.1 + 0.2}s` : '0s' }}>
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mb-3 border-4 border-purple-200 bg-white shadow-lg text-3xl transition-transform duration-200 group-hover:scale-110 animate-step-float`}
                style={{ animationDelay: `${i * 0.5}s`, backgroundColor: i === 0 ? '#ddd6fe' : i === 1 ? '#dbeafe' : i === 2 ? '#fef9c3' : '#fbbf24' }}
              >
                {step.icon}
              </div>
              ...
              {i < steps.length - 1 && (
                <div className="hidden md:block absolute top-1/2 right-[-40px] w-20 h-2 bg-gradient-to-r from-purple-100 to-blue-100 z-0" />
              )}
            </div>
          ))}
        </div>
      ...
      <section ref={testimonialsRef} className={`py-20 px-6 mb-20 bg-gradient-to-br from-purple-100 to-blue-100 text-center transition-all duration-700 ${testimonialsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
        <h2 className="text-3xl font-semibold mb-10 text-purple-600">What Clients Say</h2>
      ...
      {/* Sticky CTA bar */}
      <div className="fixed bottom-0 left-0 w-full bg-purple-600/90 shadow-lg py-4 flex justify-center items-center z-50 backdrop-blur-md">
        <span className="mr-4 font-semibold text-white">Ready to empower your people?</span>
        <a
          href="#leadform"
          className="px-6 py-3 bg-purple-500 text-white rounded-full shadow hover:bg-purple-600 hover:scale-105 transition-transform duration-200 font-semibold"
        >
          Book a Free Demo
        </a>
      </div>
